...
class PageAdapter(
    ...
    class PageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val bottomBar: LinearLayout = itemView.findViewById(R.id.bottomBar)
    }
    ...
    override fun onBindViewHolder(holder: PageViewHolder, position: Int) {
        ...
        val minBottom = (18 * density).toInt().coerceAtLeast(0)
        holder.bottomBar.setPaddingRelative(
            (insetsLeft).coerceAtLeast(minSide),
            0,
            (insetsRight).coerceAtLeast(minSide),
            (insetsBottom).coerceAtLeast(minBottom)
        )
